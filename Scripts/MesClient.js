var MesClient = function (OnOpen) {
    ...
    MesClient.prototype.Init = function (obj) {
        ...
        if (obj.OnMessage) {
            ...
            this.websocket.onmessage = function (e) {
                ...
                //$.publishMoreTime("push-message", d);
                _this.SetPushMessage(d);   //--------------
                d.MessageID && $.publish(d.MessageID, d);
            };
        }
        if (obj.OnOpen) {
    ...
    MesClient.prototype.SetUserInfo = function (e) {    //2019/05/17 modify by gch (在UserInfo上加上ip项)
        //CheckToken 請求傳回的信息 e.Data 相當於 Login 請求傳回的 e.Data.UserInfo 
        var UserInfo = e ? e.Data.UserInfo ? e.Data.UserInfo : e.Data : null; // e若為空UserInfo 為空， e若不為空就判斷e.Data.UserInfo是否存在 ，若存在則用e.Data.UserInfo，不存在則用e.Data.
        var IP = e ? e.IP : null;
        if (UserInfo) {
    ...
    MesClient.prototype.SetPushMessage = function (d){   //20200529 add by gch 在全局设置推送消息 
        this.PushMessage.push({ID: d.MessageID, Status: d.Status, StartTime: d.StartTime});
    }

    MesClient.prototype.DeletePushMessage = function (ID){   //20200529 add by gch 在全局删除推送消息 
        this.PushMessage = this.PushMessage.filter(function (msg){
            return msg.ID != ID;
        });
    }
    ...
    MesClient.prototype.GetLanguage = function (Language, PageName, CallBack) {
        var cookieName = Language + "_" + PageName;
        var cLan = localStorage.getItem(cookieName);
        //console.log(!cLan && cLan != "null", cLan);
        if (!cLan && cLan != "null") {       //无缓存，加载缓存（第一次进入页面正常，删除缓存再次刷新页面进入逻辑时Language为空，报错）
            this.CallFunction("MESStation.GlobalConfig.LanguagePageConfig", "GetPageLanguage", { PageName: PageName, LanguageValue: Language }, function (event) {
                if (event.Status == "Pass") {
                    localStorage.setItem(cookieName, JSON.stringify(event.Data));
                    CallBack(event);
            ...
            try {
                var data = JSON.parse(cLan);
                var ev = { Status: "Pass", Message: "", Data: data };
                CallBack(ev);           //刷新页面时存在缓存（正常）
            } catch (e) {                //cLan由于某种原因被置空【空字符串】（并非缓存全被删，因此缓存中存在），导致ev传入CallBack后报错（数据不符合规范）
                console.log(e);
                this.CallFunction("MESStation.Test.LanguagePageTest", "GetPageLanguage", { PageName: PageName, LanguageValue: Language }, function (event) {   //后台报错
                    localStorage.setItem(cookieName, JSON.stringify(event.data));
                    CallBack(event);
                });
            }
    ...
    MesClient.prototype.GetCodeName = function (CallBack) {   //获取验证码文件名  20190713 add by gch
        this.CallFunction("MESStation.GlobalConfig.GetCommonConfig", "GetValidCode", {}, function (e) {
            CallBack(e);
        });
    }
    ...
    MesClient.prototype.Login = function (userName, Password, Code ,CallBack) {
        this.CallFunction("MESStation.MESUserManager.UserManager", "Login", { EMP_NO: userName, Password: Password, ValidCode: Code, Language: $.cookie($.MES.CK_LAN_NAME), BU_NAME: $.cookie($.MES.CK_BU_NAME) }, function (e) {
            var tc = MesClient.prototype.ThisClient[e.ClientID];
            if (e.Status == "Pass") {
                tc.SetToken(e.Data.Token);
                //2019/05/17 modify by gch

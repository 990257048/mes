...
function save() {
    ...
    //生成Json
        ...
        MainMessage.EDIT_EMP = $('#EDIT_EMP').val();
...
function AddRouteMain() {
    $("#DeleteRouteButten").css("display", "inline-block");
    $("#AddStation").css("display", "inline-block");
    $('#RouteConfig').show();
    $("#Save").show();
    $('#RoutMain').hide();
    $('#RouteTitle').hide();
...
jsPlumb.ready(function () {
    ...
    $("#SaveStationChange").click(function () {
        ...
        self.parent.client.CallFunction("fwef", "fewfwefwe", sendData, function (e) {
            if (e.Status == "Pass") {
                //swal("修改成功！", "", "success");
                layer.msg(e.Message, {
                    icon: 1,
   ...
   // PageList("", "1");
    ...
    $('#Table').on('click-row.bs.table', function (e, row, element) {
        $("#DeleteRouteButten").css("display", "none");

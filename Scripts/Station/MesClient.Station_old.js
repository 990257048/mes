var MesStation = function (o) {
    ...
    MesStation.prototype.SendData = function () {
        ...
        if (this.StationJson.DisplayName != "SFC_LH_AGBU_AOI") {
            self.parent.Loading(true);
        }
    ...
    MesStation.prototype.CallBack = function (d) {
        ...
        if (d.Status == "Pass") {
            ...
            console.log($("#toolsite input[name=ScanType]"));
        ...
        //配置语言 2019/6/19 add by gch
        if (station.mesUI) {
            //console.log(station.mesUI);
            station.mesUI.SetLanguage("Station");
        }
        //keypart弹出框
        ...
        if (d.Data.Station.StationName == "SFC_LH_AGBU_AOI" && msgArr[0].State == 0 ) {
            $("#inputsite").find("#stop")[0].click();
        }
...
var OutputElements = function () {
    ...
    OutputElements.prototype.Table = function (c, ID, value) {        // c 输出框父容器 ID：表名 value：表数据
        ...
        if (c.attr("id") == "tabContainer") {
            tableWarp = $("<div class='tabWarp col-xs-6' style='margin-bottom:15px;'><h4  set-lan=" + "html:" + ID + ">" + ID + ":" + "</h4></div>");
        } else {
            tableWarp = $("<div class='tabWarp col-xs-12' style='margin-bottom:15px;'><h4  set-lan=" + "html:" + ID + ">" + ID + ":" + "</h4></div>");
        }

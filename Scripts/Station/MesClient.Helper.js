var Helper = function (o) {
	...
	Helper.prototype.Init = function (obj) {
		...
		if (obj.OnOpen) {
			this.websocket.onopen = function (e) {
				...
				tc.IsOpen = true;
				obj.OnOpen(e);
				var m = new StationMessage({ State: 1, Message: "Connet to MesHelper ok!" });
				m.Show("", tc.MContainer);
			}
		}
		else {
			this.websocket.onopen = function (e) {
				if ($.MES.DEBUG) {
				    console.log("onOpen:MesHelper connetion open");
				}
				var tc = Helper.prototype.ThisClient[this.ClientID];
				tc.IsOpen = true;
				var m = new StationMessage({ State: 1, Message: "Connet to MesHelper ok!" });
		...
		if (obj.OnClose) {
			...
			this.websocket.onclose = function (e) {
				...
				m.Show("", tc.MContainer);
				if ($.MES.DEBUG) {
				    console.log("onClose:MesHelper connetion close");
				}
			};
		}
		if (obj.OnError) {
			...
			this.websocket.onerror = function (e) {
				if ($.MES.DEBUG) {
				    console.log("onError: MesHelper connetion Fail!");
				}
	...
	Helper.prototype.CallFunction = function (TCode, Data, CallBack, MessageID) {
		...
		var data = {BU: this.BU, ClientID: this.ClientID, MessageID: MessageID, TCode: TCode, Data: Data };
    ...
    /////////////////////////GCH 2018/10/23 UPDATE/////////////////
	Helper.prototype.ReadData = function (Data,CallBack) {
	    this.CallFunction("GETCOMDATA",Data, CallBack);
	};
    //////////////////////////GCH 2018/10/23 ADD///////////////////
	Helper.prototype.ReadTxtListData = function (Data, CallBack) {
	    this.CallFunction("GETTXTDATA", Data, CallBack);
	};

	Helper.prototype.ReadAOITxtListData = function (Data, CallBack) {
	    this.CallFunction("GETAOITXTDATA", Data, CallBack);
	};

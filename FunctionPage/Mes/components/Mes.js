let { useState, useEffect, useMemo, useCallback, memo, useRef } = React;
...
let Mes = () => {
    ...
    useMemo(() => {
        $.subscribe("add_iframe", (ev, o) => {    //添加选项卡的接口    o: { name, url }
            dispatch($open_iframe(o.name, o.url));
        });
    }, []);
    return <div id="app">
        <MesTop />
...
let MesNav = () => {
    ...
    return <div className="mes-nav">
        ...
        <ul className="nav-ul">
            {
                iframes.map(({ name, url }) => {
                    return <NavBar key={url} nav_d={{ name: name, act_flag: iframe_act_url == url ? true : false }} />
                })
            }
        </ul>
        ...
        <div className="mes-exit" set-lan="html:exit" onClick={exit_fn}> 退出 </div>
    </div>
}


let NavBar = ({ nav_d }) => {
    let { name, act_flag } = nav_d;
    let { iframes, iframe_act_url } = useSelector(state => state.MesIframesReducer);
    let dispatch = useDispatch();

    let click_fn = useCallback((ev) => {
        if ($(ev.target).attr("class") != "close-bar-icon") {
            dispatch(change_act_iframe(name));
        }
    }, []);

    let nav_close = useCallback((ev) => {
        var c_flag = false;  //点击的选项卡是否是当前活动的 
        iframes.forEach(navO => {
            if (navO.name == name && navO.url == iframe_act_url) {
                c_flag = true;
            }
        });
        if (c_flag) {
    ...
    if (name == '首页') {
        return <li className={act_flag ? "nav-act" : null} onClick={click_fn}>
            {name}
        </li>
    } else {
        return <li className={act_flag ? "nav-act" : null} onClick={click_fn}>
            {name}
            <span className="close-bar-icon" onClick={nav_close}></span>
        </li>
    }
}

let MesIframeWrap = () => {
    let iframes = useSelector(state => state.MesIframesReducer.iframes);
...
let MesIframe = ({url}) => {
    let iframe_act_url = useSelector(state => state.MesIframesReducer.iframe_act_url);
    let display = useMemo(() => {
        return iframe_act_url == url ? "block" : "none";
    }, [iframe_act_url, url]);
    if (url == "menu") {

...
let WrapCon6 = () => {
    ...
    let [tableConfig, setTableConfig] = useState([]);
    let dispatch = useDispatch();
    ...
    const dataSource = [
        {
            ID: "MFGII000000000000000000000000000DEJ",
            BU: "MFGII",
            SKUNO: "01-123456-11",
            RATE: 0.0,
            STATION_NAME: "VI",
    ...
    useEffect(() => {
        ...
        SKUNO != "" && get2dxConfig({ SKUNO }).then(e => {
            setTableConfig( ret_table_config(e.Data) );    //设置表格
            // if (e.Data[0]) {
    ...
    let clickSave2dxConfig = () => {
        ...
        save2dxConfig({ BU, SKUNO, CODE_NAME: SKU_NAME, STATION_NAME, QTY }).then(e => {    // 不传是否有效
            SKUNO != "" && get2dxConfig({ SKUNO }).then(e => {
                setTableConfig( ret_table_config(e.Data) );
            }, e => {
                message.error(e.Message);
            });
            message.success(e.Message);
    ...
    let delete2dxConfig = (ID) => {    // 删除2dx配置回调
        delete2dxConfigById({ID}).then(e => {
            SKUNO != "" && get2dxConfig({ SKUNO }).then(e => {
                setTableConfig( ret_table_config(e.Data) );
            }, e => {
                message.error(e.Message);
    ...
    let ret_table_config = (dataSource) => ({
        size: 'small',
        // bordered: true,
        pagination: {
            size: 'small',
        ...
        columns: [
            ...
            },
            {
                ...
                render: (r) => {
                    // console.log(r);
                    return <Space>
                        <Popconfirm placement="topRight" title="你确定要删除该配置吗？" okText="Yes" cancelText="No"
                        // onCancel={} 
                        onConfirm={() => {
                            delete2dxConfig(r.ID);
                        }}
                        >
    ...
    return (<div className="wrap-con6">
        ...
        <div className="con-son">
            ...
            <Input size="middle" 
                // value={QTY} 
                onChange={e => { dispatch(set_con_init("con6", { QTY: e.target.value })) }} 
            />
        </div>
